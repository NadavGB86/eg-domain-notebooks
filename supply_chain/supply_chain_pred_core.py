...
class SupplyChainModelTrainer:
    ...
    def optimize_hyperparameters(self, X: pd.DataFrame, y: pd.Series, weights: pd.Series,
                                 categorical_cols: list, n_trials: int = 50,
                                 custom_search_space: Callable = None,
                                 pruning_metric: str = 'Procurement_NDCG') -> dict:
        ...
        default_metric_ = 'binary_logloss'  # Default fallback

        def objective(trial):
            base_params = {
                'verbosity': -1, 'random_state': self.random_state, 'objective': self.objective_func,
            }
            if isinstance(self.objective_func, str) and 'tweedie' in self.objective_func.lower():
                default_metric_ = 'rmse'
                base_params['metric'] = default_metric_

            # Inject custom search space if provided, else use default classification space
            param_grid = {}
            if custom_search_space:
                param_grid.update(custom_search_space(trial))
            else:
                # Default fallback (Legacy Classification Space)
                param_grid.update({
            ...
            for train_idx, val_idx in cv.split(X, y):
                ...
                model = lgb.train(
                    params=params, train_set=dtrain, num_boost_round=250, valid_sets=[dval],
                    feval=self.eval_metric_func,
                    callbacks=[
                        lgb.early_stopping(stopping_rounds=25, verbose=False),
                        LightGBMPruningCallback(trial, pruning_metric, valid_name='valid_0'),
                    ],
                )

                # Dynamic metric extraction
                score = model.best_score['valid_0'].get(pruning_metric, None)
                if score is None:
                    raise KeyError(
                        f"Metric '{pruning_metric}' not found in evaluation results. Check your eval_metric_func.")
                cv_scores.append(model.best_score['valid_0'][pruning_metric])

            return np.mean(cv_scores)

        # Logic for minimization vs maximization based on metric
        direction = "minimize" if pruning_metric.lower() in ['rmse', 'logloss', 'mae'] else "maximize"
        ...
        logger.info(f"[*] Optimization Complete. Best OOF Metric: {study.best_value:.4f}")
        logger.info(f"[*] Best Params:\n{study.best_params}")
        self.best_params_ = copy.deepcopy(study.best_params)

        # The Objective Injection Fix (Ensuring Final Model works)
        self.best_params_ = study.best_params
        self.best_params_['objective'] = self.objective_func
        self.best_params_['metric'] = default_metric_
...
# =============================================================================
# CONFORMAL REGRESSION BUFFER
# =============================================================================

class ForecastingConformalEngine:
    """
    A Conformal Prediction engine specifically built for continuous regression tasks.
    It calculates the upper bound of uncertainty in physical units, not probabilities.
    """
    ...
    def calibrate(self, y_true: np.ndarray, y_pred: np.ndarray):
        """
        Calibrates the engine on the validation set.
        Finds the exact error threshold that covers (1 - alpha)% of past mistakes.
        """
        residuals = np.abs(y_true - y_pred)
        n = len(residuals)

        # Conformal quantile formula
        q = (n + 1) * (1 - self.alpha) / n
        q = np.clip(q, 0.0, 1.0)  # Ensure it strictly remains a valid quantile

        # tau_hat is the physical number of units our model is typically wrong by
        self.tau_hat = np.quantile(residuals, q)
        return self.tau_hat

    def calculate_safety_buffer(self, base_forecasts: pd.Series, lead_time_weeks: pd.Series = None) -> pd.DataFrame:
        """
        Translates the conformal uncertainty directly into a Safety Stock Buffer.
        """
        if self.tau_hat is None:
            raise ValueError("Engine must be calibrated before generating recommendations.")

        decisions = pd.DataFrame(index=base_forecasts.index)
        decisions['Base_Forecast'] = base_forecasts

        # The base conformal interval. If lead time is provided, scale the uncertainty.
        # Assuming simple square-root scaling for time (standard supply chain physics)
        lt_multiplier = np.sqrt(lead_time_weeks) if lead_time_weeks is not None else 1.0

        decisions['Safety_Buffer_Units'] = np.ceil(self.tau_hat * lt_multiplier)
        decisions['Total_Required_Inventory'] = np.ceil(
            decisions['Base_Forecast'] + decisions['Safety_Buffer_Units'])
